...
use crate::msg::{
    ConfigResponse, ExecuteMsg, MintCountResponse, MintData, MintPriceResponse, MintWhitelist, MintableNumTokensResponse, QueryMsg, StartTimeResponse
};
use crate::state::{
    Config, ConfigExtension, CONFIG, MERKLE_ROOT, MERKLE_TREE_URI, MINTABLE_NUM_TOKENS, MINTED_IDS, MINTER_ADDRS, SG721_ADDRESS, STATUS
};
...
use cosmwasm_std::{
    ...
    Empty, Env, Event, MessageInfo, Reply, ReplyOn, StdError, StdResult, SubMsg,
...
use sg_std::{GENESIS_MINT_START_TIME, NATIVE_DENOM, Response};
use sg_whitelist::msg::{
    ConfigResponse as WhitelistConfigResponse, HasMemberResponse, QueryMsg as WhitelistQueryMsg,
};
//use sha2::{Digest, Sha256};
use metadata_factory::{
    msg::{ParamsResponse, MetadataMinterCreateMsg},
    state::MetadataMinterParams
};
...
pub fn instantiate(
    ...
    MERKLE_ROOT.save(deps.storage, &msg.init_msg.merkle_root)?;
    let tree_url = msg.init_msg.merkle_tree_uri.unwrap_or_default();
    MERKLE_TREE_URI.save(deps.storage, &tree_url)?;
    ...
    let config = Config {
        ...
        extension: ConfigExtension {
            ...
            discount_price: None,
            merkle_root: msg.init_msg.merkle_root.clone(),
        },
    ...
    let submsg = SubMsg {
        msg: WasmMsg::Instantiate {
            ...
            msg: to_json_binary(&sg721_metadata_onchain::InstantiateMsg {
    ...
    Ok(Response::new()
        ...
        .add_attribute("merkle_root", msg.init_msg.merkle_root)
        .add_attribute("merkle_tree_uri", tree_url)
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::Mint { 
            mint_data, 
            whitelist 
        } => execute_mint_sender(deps, env, info, mint_data, whitelist),
        ...
        ExecuteMsg::UpdatePerAddressLimit { per_address_limit } => {
         ...
         ExecuteMsg::MintTo { 
            mint_data, 
            recipient 
        } => execute_mint_to(deps, env, info, recipient, mint_data),
        ExecuteMsg::MintFor { 
            mint_data, 
            recipient 
        } => execute_mint_to(deps, env, info,  recipient, mint_data),
...
pub fn execute_purge(
    ...
    MINTER_ADDRS.clear(deps.storage);
    MINTED_IDS.clear(deps.storage);
...
pub fn execute_mint_sender(
    ...
    if is_public_mint(deps.as_ref(), &info, whitelist.clone())?
    ...
    let allowance = whitelist.map(|w| w.allowance).flatten();
    
    if mint_count >= allowance.unwrap_or(config.extension.per_address_limit) {
...
fn is_public_mint(
    ...
    let wl_contract_addr = config.extension.whitelist.unwrap();

    let wl_config: WhitelistConfigResponse = deps
        .querier
        .query_wasm_smart(wl_contract_addr.clone(), &WhitelistQueryMsg::Config {})?;
    ...
    let minter_whitelist = whitelist.unwrap();


    let res: HasMemberResponse = if  minter_whitelist.proof_hashes.is_some() || is_merkle_tree_wl(&wl_config) {

        let member = if minter_whitelist.allowance.is_some() {
            format!("{}:{}", info.sender, minter_whitelist.allowance.unwrap())
        } else {
        ...
        deps.querier.query_wasm_smart(
            ...
            &WhitelistMtreeQueryMsg::HasMember {
                member,
                proof_hashes: minter_whitelist.proof_hashes.unwrap(),
            },
        )?
...
pub fn execute_mint_to(
    ...
    _execute_mint(deps, env, info, action, true, Some(recipient), data)
}
...
fn _execute_mint(
    ...
    MINTABLE_NUM_TOKENS.save(deps.storage, &(mintable_count - 1))?;
    ...
    mint_data.check_inclusion(deps.storage)?;
...
pub fn execute_update_start_trading_time(
    ...
    let msg = WasmMsg::Execute {
        ...
        msg: to_json_binary(&sg721_metadata_onchain::ExecuteMsg::UpdateStartTradingTime(
            start_time,
        ))?,
        funds: vec![],
    };
...
fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    Ok(ConfigResponse {
        ...
        merkle_tree_uri: MERKLE_TREE_URI.load(deps.storage)?,
    })
}
